...
Install the repo hook once per clone:

```bash
npm run hooks:install
```

The pre-push hook runs `npm run check:package`, which rebuilds the installable bundles and fails if `dist` or `dist-server` changed. When that happens, add and commit the generated bundle changes before pushing so `codex-web-ui --update` installs the current UI.
