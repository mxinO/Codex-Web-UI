...
export function codexBackfillBusyStartupLine(line: string): boolean {
  return line.includes('timed out waiting for state db backfill at ') && line.includes('(status: running)');
}

export function isCodexBackfillBusyStartupError(error: unknown): error is CodexBackfillBusyStartupError {
  return error instanceof CodexBackfillBusyStartupError;
}

export function resolveCodexAppServerStartupTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
...
export function prepareCodexChildRuntimeEnv(
  ...
  if (preserveNodeWebApis(childEnv)) return { env: childEnv, nodeWrapperPath: null, cleanup: () => undefined };

  const wrapper = createNodeWrapper(realNode, platform);
  if (!wrapper) return { env: childEnv, nodeWrapperPath: null, cleanup: () => undefined };
...
export function codexAppServerArgs(sqliteHome?: string): string[] {
...
export class CodexAppServer {
  ...
  private startReal(): Promise<CodexInitializeResponse> {
    return new Promise<CodexInitializeResponse>((resolve, reject) => {
      ...
      const handleOutputLine = (stream: 'stdout' | 'stderr', line: string) => {
        ...
        if (stream === 'stderr' && codexBackfillBusyStartupLine(line)) backfillBusy = true;
  ...
  private async connect(url: string, isCancelled: () => boolean): Promise<CodexInitializeResponse> {
    ...
    const response = await peer.request<CodexInitializeResponse>('initialize', {
