...
export type TurnRuntimeContextLookup =
  | { status: 'found'; context: TurnRuntimeContext; scannedBytes: number }
  | { status: 'none' | 'unavailable' | 'scanLimit'; context: null; scannedBytes: number; detail?: string };

export interface TurnRuntimeContextReadOptions {
  maxScanBytes?: number;
  chunkBytes?: number;
}
...
function contextFromLine(line: Buffer): TurnRuntimeContext | null {
  const content = line.at(-1) === 0x0d ? line.subarray(0, -1) : line;
  if (content.length === 0 || content.indexOf(TURN_CONTEXT_MARKER) === -1) return null;

  try {
    const entry = JSON.parse(content.toString('utf8')) as unknown;
    if (!entry || typeof entry !== 'object') return null;
    const record = entry as Record<string, unknown>;
    if (record.type !== 'turn_context' || !record.payload || typeof record.payload !== 'object') return null;
...
async function readCompleteChunk(
...
export async function readLatestTurnRuntimeContext(
  rolloutPath: string | null,
  options: TurnRuntimeContextReadOptions = {},
): Promise<TurnRuntimeContextLookup> {
  if (!rolloutPath) return { status: 'none', context: null, scannedBytes: 0 };
  ...
  let lookup: TurnRuntimeContextLookup;
  try {
    const { size } = await handle.stat();
    let position = size;
    let carry = Buffer.alloc(0);
    let found: TurnRuntimeContext | null = null;
