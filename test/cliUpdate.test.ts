...
describe('CLI update helper', () => {
  it('builds a global npm install for the default GitHub tarball', async () => {
    ...
    expect(spawn).toHaveBeenCalledWith('npm', ['install', '-g', '--dangerously-allow-all-scripts', DEFAULT_UPDATE_SOURCE], expect.objectContaining({
  ...
  it('accepts an explicit update source', async () => {
    ...
    await expect(resultPromise).resolves.toBe(0);
    expect(spawn).toHaveBeenCalledWith('npm', ['install', '-g', '--dangerously-allow-all-scripts', 'file:///tmp/codex-web-ui.tgz'], expect.any(Object));
  });
  ...
  it.each(['EISDIR', 'ENOTEMPTY'])(
    ...
    async (errorCode) => {
      ...
      expect(spawn).toHaveBeenNthCalledWith(
        1,
        'npm',
        ['install', '-g', '--dangerously-allow-all-scripts', DEFAULT_UPDATE_SOURCE],
