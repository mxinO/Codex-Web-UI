...
function statusKind(indexStatus, worktreeStatus) {
...
function pushEntry(entries, entry, limit) {
    if (entries.length >= limit)
        return false;
    entries.push(entry);
    return true;
}
function parseBranchHeader(result, record) {
...
function parseRename(record, originalPath) {
    const body = record.slice(2);
    const xy = body.slice(0, 2);
    const fields = body.slice(3).split(' ');
    if (fields.length < 8)
...
export function parseGitStatusPorcelainV2(output, options = {}) {
