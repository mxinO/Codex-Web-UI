...
function expectedGoalFromParams(params, requireStatus) {
    if (!isRecord(params) || !hasOwn(params, 'expectedGoal'))
        return { error: 'expectedGoal is required' };
    if (params.expectedGoal === null)
        return { value: null };
    if (!isRecord(params.expectedGoal))
        return { error: 'expectedGoal must be a goal fingerprint or null' };
    ...
    if (!objective || createdAt === null)
        return { error: 'expectedGoal must include objective and createdAt' };
    if (!requireStatus)
        return { value: { objective, createdAt } };
    const status = enumValue(params.expectedGoal.status, GOAL_STATUSES);
    if (!status)
        return { error: 'expectedGoal must include status for replacement' };
    return { value: { objective, createdAt, status } };
}
function goalMatchesFingerprint(goal, expected) {
    if (!goal || !expected)
        return goal === null && expected === null;
    return (goal.objective === expected.objective &&
...
export function attachBrowserSocket(server, deps) {
    ...
    const suppressedGoalQueueStarts = new Set();
    const suppressedGoalQueueSteers = new Set();
    let compactionGeneration = 0;
    const compactionsInFlight = new Map();
    ...
    const rememberTerminalTurnKey = (key, disposition) => {
    ...
    const rememberAutonomousSuccessor = (threadId, predecessorTurnId, successorTurnId) => {
        const predecessorKey = turnKey(threadId, predecessorTurnId);
        if (autonomousSuccessorTurnIds.has(predecessorKey))
            return;
    ...
    const terminalDispositionForTurn = (threadId, turnId) => terminalTurnDispositions.get(turnKey(threadId, turnId)) ??
        terminalTurnDispositions.get(turnKey(null, turnId)) ??
        null;
    const appendAutonomousSuccessor = (threadId, rootTurnId, successorTurnId) => {
        let predecessorTurnId = rootTurnId;
        const visited = new Set();
        while (!visited.has(predecessorTurnId)) {
            if (predecessorTurnId === successorTurnId || terminalDispositionForTurn(threadId, predecessorTurnId) === 'barrier')
                return;
    ...
    const autonomousSuccessorDisposition = (steer) => {
    ...
    const turnStartInFlightForThread = (threadId) => {
        if (pendingTurnStartContexts.some((context) => context.threadId === threadId))
            return true;
        const state = deps.stateStore.read();
        return state.activeThreadId === threadId && isPendingTurnStartForThread(state.activeTurnId, threadId);
    };
    const compactionInFlightForThread = (threadId) => {
        ...
        void (async () => {
            ...
            try {
                ...
                if (terminalDisposition === 'barrier') {
                    ...
                    else if (current.activeThreadId === claimToSteer.threadId &&
                        current.activeTurnId &&
    ...
    const wakeQueuedWorkAfterGoalUpdate = (threadId, options) => {
        queueMicrotask(() => {
            if (goalUpdates.has(threadId))
                return;
            ...
            if (options.allowSteer && steerWasSuppressed)
                maybeSteerQueuedMessage();
            if (options.allowStart && startWasSuppressed)
    ...
    const markQueuedMessageMaybeSent = (threadId, queuedMessageId) => {
        ...
        const next = deps.stateStore.update((state) => {
            const queue = state.queue.map((message) => {
                if (message.id !== queuedMessageId || message.threadId !== threadId)
                    return message;
    ...
    const resumeQueuedWorkAfterSettledCompletionSteer = (steer) => {
        if (steer.timedOut) {
            const next = markQueuedMessageMaybeSent(steer.threadId, steer.queuedMessage.id);
            if (next)
                broadcastHello(next);
        }
        if (autonomousSuccessorDisposition(steer) === 'barrier')
            return;
        const current = deps.stateStore.read();
    ...
    const handleTurnCompleted = async (message, disposition) => {
        ...
        try {
            ...
            const matchedCurrentActiveTurnAtReceipt = Boolean(current.activeTurnId) &&
            ...
            if (noTurnVerificationKey) {
                ...
                if (remainingOwners > 0)
                    verifyingUnscopedTerminalKeys.set(noTurnVerificationKey, remainingOwners);
                else
            ...
            if (compactedThreadId && compaction) {
                ...
                const boundNoIdCompactedNotification = message.method === 'thread/compacted' &&
                    !completedTurnId &&
                ...
                const unboundNoIdCompactedNotification = message.method === 'thread/compacted' &&
                    !completedTurnId &&
                    compaction.turnId === null &&
                    receiptState.activeThreadId === compactedThreadId &&
    ...
    const handleTaskStarted = (message) => {
        ...
        if (!canAdoptTurn(current))
            return;
        let adopted = false;
        let adoptedPredecessorTurnId = null;
        const terminalPredecessorTurnId = queuedSteerInFlight?.threadId === threadId &&
        ...
        ws.on('message', async (raw) => {
            ...
            try {
                ...
                if (request.method === 'webui/thread/compact/start') {
                    ...
                    const compaction = { generation: (compactionGeneration += 1), turnId: null };
                    ...
                    try {
                        ...
                        if (compactionsInFlight.get(threadId)?.generation === compaction.generation)
                            compactionsInFlight.delete(threadId);
                ...
                if (request.method === 'webui/thread/goal/replace' || request.method === 'webui/thread/goal/edit') {
                    ...
                    try {
                        ...
                        const reconcileGoal = async (shouldRetry) => {
                            let reconciled = null;
                            for (let attempt = 0; attempt < 3; attempt += 1) {
                                if (attempt > 0)
                                    await new Promise((resolve) => setTimeout(resolve, attempt * 25));
                                reconciled = await readGoal();
                                if (!shouldRetry(reconciled))
                                    break;
                            }
                            return reconciled;
                        };
                        ...
                        const validateReplacementGoal = (candidate, requireNewIdentity) => {
                            if (!candidate)
                                throw new Error('goal replacement is incomplete; no goal is currently set');
                        ...
                        if (request.method === 'webui/thread/goal/edit') {
                            try {
                                ...
                                const reconciled = await reconcileGoal((goal) => goal === null || (goal.createdAt === expected.value?.createdAt && goal.objective !== objective));
                                if (!reconciled)
                                    throw new Error('goal edit failed because the goal no longer exists');
                                if (reconciled.createdAt !== expected.value?.createdAt)
                                    throw new Error('goal edit conflicted with a different goal');
                                if (reconciled.objective !== objective)
                                    throw new Error('goal edit was not applied; the existing goal remains');
                            ...
                            try {
                                ...
                                const reconciled = await reconcileGoal((goal) => goal === null || Boolean(currentGoal && goal.createdAt === currentGoal.createdAt));
                        ...
                        if (next)
                            broadcastHello(next);
                        if (request.method === 'webui/thread/goal/replace')
                            validateReplacementGoal(goal, false);
                        send(ws, { type: 'rpc/result', id: request.id, result });
                    }
                    finally {
                        goalUpdates.delete(threadId);
