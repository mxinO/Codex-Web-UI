...
export function sessionFileEditDbPath(threadPath) {
...
export class FileEditStore {
    ...
    recordSnapshot(input) {
        ...
        const insert = this.db.transaction(() => {
            this.db
                .prepare(`INSERT INTO file_edit_events
           ...
           VALUES (?, ?, ?, ?, ?, NULL, 'snapshot', ?, ?)`)
            ...
            this.db
                .prepare(`INSERT INTO turn_file_diffs
    ...
    discardPatchDiff(input) {
        const hash = pathHash(input.path);
        const discard = this.db.transaction(() => {
            const fallback = this.db
                .prepare(`SELECT path, before_text, source
           FROM file_edit_events
           WHERE turn_id = ? AND path_hash = ? AND source != 'patch'
           ORDER BY id ASC
           LIMIT 1`)
                .get(input.turnId, hash);
            if (fallback) {
                this.db
                    .prepare(`UPDATE turn_file_diffs
            ...
            this.db
                .prepare(`DELETE FROM turn_file_diffs
           WHERE turn_id = ? AND path_hash = ? AND source = 'patch'`)
    ...
    finalizeFile(input) {
        ...
        const finalize = this.db.transaction(() => {
           ...
           ON CONFLICT(turn_id, path_hash) DO UPDATE SET
            ...
            this.db
                .prepare(`UPDATE file_edit_events
    ...
    getDiff(turnId, filePath) {
        const row = this.db
            .prepare(`SELECT turn_id, path, before_text, after_text, edit_count, updated_at_ms
         FROM turn_file_diffs
         WHERE turn_id = ? AND path_hash = ?`)
            .get(turnId, pathHash(filePath));
        if (!row || row.after_text === null)
            return null;
        return {
            turnId: row.turn_id,
            path: row.path,
    ...
    getSnapshot(turnId, filePath) {
        const row = this.db
            .prepare(`SELECT path, before_text, edit_count, source
         FROM turn_file_diffs
         WHERE turn_id = ? AND path_hash = ?`)
            .get(turnId, pathHash(filePath));
        if (!row)
    ...
    listTurnFiles(turnId) {
        const rows = this.db
            .prepare(`SELECT turn_id, path, after_text, edit_count, updated_at_ms
         FROM turn_file_diffs
         WHERE turn_id = ?
